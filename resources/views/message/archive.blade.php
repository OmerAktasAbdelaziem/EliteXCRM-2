    ...
    @if ($archive_counts <= 0)
        <div class="page-wrapper">
            <div class="page-content">
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title">Archive messages</h5>
                        <p class="card-text text-center">You don't have any archived messages yet !</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="page-content">
                <div class="chat-wrapper my-5">
                                    ...
                                    <div class="chat-list">
                                        ...
                                        <ul class="p-0">
                                            @foreach ($messages as $messag)
                                                @php
                                                    if ($message->message->sender->lastseen_at != null) {
                                                ...
                                                @endphp
                                                <li>
                                                    <a href="{{ route('messages.archive' ,$messag->id) }}" class="list-group-item">
                                                        <div class="d-flex">
                                                            ...
                                                            <div class="flex-grow-1 ms-2">
                                                                <h6 class="mb-0 chat-title">{{$messag->message->sender->first_name}} {{$messag->message->sender->last_name}}</h6>
                                                                <p class="mb-0 chat-msg">{{substr($messag->message->text, 0, 25)}}.</p>
                                                            </div>
                                                            <div class="chat-time">{{ $messag->formatted_date }}</div>
                                                        </div>
                                                    </a>
                                                </li>
                                            @endforeach
                    ...
                    <div class="chat-header d-flex align-items-center">
                        <form action="{{ route('archives.destroy',$message->id) }}" method="POST" name="archiveform" id="archiveform">
                            @csrf
                            @method('DELETE')
                        </form>
                        ...
                        <div>
                            <h4 class="mb-1 font-weight-bold">{{$message->message->sender->first_name}} {{$message->message->sender->last_name}} ({{$message->message->sender->username}})</h4>
                        ...
                        <div class="chat-top-header-menu ms-auto"> <a href="javascript:;" onclick="$('#archiveform').submit();"><i class="bx bx-archive-out"></i></a>
                        </div>
                    </div>
                    <div class="chat-content">
                        <div class="chat-content-leftside">
                            <div class="d-flex">
                                @if ($message->message->sender->gender && $message->message->sender->gender == 'Female')
                                ...
                                @endif
                                <div class="flex-grow-1 ms-2">
                                    <p class="mb-0 chat-time">{{$message->message->sender->first_name}}, {{$message->message->created_at}}</p>
                                    <p class="chat-left-msg">{{$message->message->text}}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="chat-footer d-flex align-items-center">
                        <div class="flex-grow-1 pe-2">
                            <div class="input-group">	<span class="input-group-text"><i class='bx bx-message'></i></span>
                                <form action="{{ route('message.store',$message->message->sender->id ) }}" class="hidden" id="newmassage" name="newmassage" method="POST">
                                    @csrf
                                </form>
