                ...
                <div class="d-flex">
                    <div class="flex-grow-1 ms-2">
                        <form class="ajax-form" method="POST" action="{{ route('client-comments.update',$comment->id) }}">
                            ...
                            <p class="mb-0 chat-time"><a @if ($isSuperAdmin || $isPipelineAdmin || UserPermission::hasPermissionInPipeline($userAuth, $pipelineId, 'users_show')) href="{{ route('user.show',$comment->user_id ) }}" @endif>{{$comment->user->first_name}} ({{$comment->user->username}})</a>, {{ date('d/m/Y H:i', strtotime($comment->created_at)) }} ,<a @if ($isSuperAdmin || $isPipelineAdmin || UserPermission::hasPermissionInPipeline($userAuth, $pipelineId, 'leads_show')) href="{{ route('client.show',$comment->client_id ) }}" @endif>{{$comment->client->first_name}}</a></p>
...
<script>
    let users_show = @json(($isSuperAdmin || $isPipelineAdmin || UserPermission::hasPermissionInPipeline($userAuth, $pipelineId, 'users_show')));
    let leads_show = @json(($isSuperAdmin || $isPipelineAdmin || UserPermission::hasPermissionInPipeline($userAuth, $pipelineId, 'leads_show')));
